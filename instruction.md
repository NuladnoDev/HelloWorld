# instruction.md — инструкция для следующей нейросети по проекту hw-core

Этот файл объясняет, как устроен проект `hw-core`: его Rust‑ядро, Qt‑клиент, интеграция с Supabase, где лежат ключи и какие инварианты нельзя ломать.
...
- **Rust‑crate `hw_core`** — криптографическое ядро:
  ...
  - FFI‑слой для вызова из C++.
- **C++/Qt‑клиент `cpp_client`**:
  - UI‑слой на `QMainWindow` (регистрация и чат);
  - обёртка над Supabase REST/Realtime (`SupabaseClient`);
  - glue‑код между UI и Rust‑ядром (через `rust_core.h`).
